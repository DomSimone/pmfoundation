document.addEventListener('DOMContentLoaded', () => {
    ...
    // Organisational Structure diagram - fit into the page margins and align with the text layout
    const structureDiagram = document.querySelector('#org-structure .structure-diagram');
    if (structureDiagram) {
        const applyStructureAlignment = () => {
            const section = document.getElementById('org-structure');
            if (!section) return;
            const sectionStyle = getComputedStyle(section);
            const pl = parseFloat(sectionStyle.paddingLeft) || 0;
            const pr = parseFloat(sectionStyle.paddingRight) || 0;
            const textColumnWidth = Math.max(0, section.getBoundingClientRect().width - pl - pr);
            structureDiagram.style.maxWidth = textColumnWidth + 'px';
            structureDiagram.style.height = 'auto';
            structureDiagram.style.display = 'block';
            structureDiagram.style.margin = '25px auto 15px';
        };
        const initStructureDiagram = () => {
            applyStructureAlignment();
            if (!structureDiagram.complete) {
                structureDiagram.addEventListener('load', applyStructureAlignment);
            }
            window.addEventListener('resize', applyStructureAlignment);
        };
